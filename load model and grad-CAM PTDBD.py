...
pred = np.squeeze(np.transpose(X_pred[case]))
grad = np.transpose(grad_top1)
...
for idx,ax in enumerate(ax_list.T.flatten()):
   ...
   ax.plot(np.transpose(pred[idx]),linewidth=2,color='black')
...
plt.suptitle("(casenr {} class {} predicted class {})".format(case,a[case],Y_pred[case]))
